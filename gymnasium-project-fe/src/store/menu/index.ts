...
//动态生成路由
export function generateRoutes(routes: RouteRecordRaw[], router: any, isTopLevel: boolean = true) {
    //定义接收生成的菜单
    ...
    routes.forEach((route: any) => {
        ...
        if (tmp.children && tmp.children.length > 0) {
            ...
            //递归处理children，传入false表示不是顶层
            tmp.children = generateRoutes(tmp.children, router, false)
        }
        //只有顶层路由才 addRoute，子路由已嵌套在父路由children中
        if (isTopLevel) {
            router.addRoute(tmp)
        }
        res.push(tmp)
